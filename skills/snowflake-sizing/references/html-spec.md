...
SEs share the proposal as a PDF. There are two paths: a primary **Download PDF** button (in-browser, no print dialog) and a secondary **Print** fallback that uses the native browser dialog.

### Download PDF button (primary)

A floating `<button class="pdf-btn" onclick="downloadPDF()">Download PDF</button>` is rendered at fixed top-right (navy, primary). `downloadPDF()` loads `html2pdf.js` (bundled via the same jsDelivr CDN as Chart.js), adds the `body.pdf-export` class, reflows the charts, then rasterizes `.container` into a multi-page A4 PDF with no browser-injected headers or footers. The filename reuses the slug/version/date convention from `saveSnapshot()` with a `.pdf` extension. The class is removed in a `finally{}` block so the page always returns to its interactive state.

The `body.pdf-export` CSS block is the screen-mode equivalent of `@media print`: `html2canvas` does NOT honor `@media print`, so the two blocks must be kept in sync as a maintenance pair. It expands all tabs in flow, renders `data-print-title` headers, hides interactive chrome (including `.pdf-btn`, `.print-btn`, `.save-btn`), and neutralizes form controls. **Pagination intentionally diverges from `@media print`:** the native print path puts each tab on its own page (a per-tab handout), but html2pdf slices the rasterized `.container` into A4 strips, so forcing a page per tab produced ~15 mostly-empty pages. The PDF block therefore flows tabs continuously (`.tab-content + .tab-content { page-break-before: auto; }`) and relies on `page-break-inside: avoid` / `break-inside: avoid` on `.section`, `.workload-card`, `.scenario-card`, `.chart-row`, `.chart-container`, and `.kpi-tile` so a slice boundary never cuts a chart or tile in half. The tab `::before` heading also gets `page-break-after: avoid` so a section title never orphans at the bottom of a page.

**Horizontal width guard (fixed-width capture).** html2pdf paginates vertically only; it has no horizontal pagination, so any content wider than the captured `.container` is silently clipped at the right page edge. The earlier `.container { max-width: 100% }` let capture width float with the (variable) browser window, which clipped the 4th KPI tile, the Year 2/3 chart bars, and long warehouse titles/`cr/hr` formulas. The fix pins capture to A4 content width (794px @96dpi): `body.pdf-export .container { width: 794px; max-width: 794px }`. **Critically, the `html2canvas` options matter and are easy to get wrong on wide displays.** In a real wide browser (e.g. a 1440–1680px laptop), html2canvas computes a too-narrow output canvas (~719px for a 794px element) and clips the right ~75px of every page; and naive `windowWidth: 794` / `width: 794` overrides instead re-map the capture origin against the live wide-window coordinates, shifting content *off the left edge* and leaving the right ~half of every page blank. The working combination (in `downloadPDF()`) is all three of: `width: 794` (forces the output canvas to A4 content width) + `windowWidth: window.innerWidth` (keep the layout at the REAL window width so html2canvas's origin math stays aligned — do NOT pin this narrower) + an `onclone` callback that sets `width:794px; max-width:794px; margin:0` on the cloned `<html>`, `<body>`, and `.container`. Together these make the captured content fill exactly `0..794` with no left inset or right clip, deterministically across window widths. Three supporting rules prevent child overflow at that fixed width: `.kpi-grid` uses `repeat(4, minmax(0, 1fr))` (default `min-width:auto` would force the 4 tiles to overflow); `.workload-header` gets `flex-wrap: wrap` with `.card-label-edit { flex: 1 1 100% }` and `.workload-calc { white-space: normal; word-break: break-word }` so the monospace formula drops to its own line instead of clipping; and `canvas { max-width: 100% !important }` caps any stale chart backing-store width. The KPI `minmax(0,1fr)` and `.workload-header` wrap rules are mirrored into `@media print` (the `.container` width is unnecessary there since `@page` controls the page box). **When changing the PDF capture, always verify in a wide (≥1440px) headless browser — a narrow viewport hides this class of bug.**

### Print button (fallback)

A floating `<button class="print-btn" onclick="window.print()">Print</button>` opens the native browser print dialog. Hidden by `@media print` and `body.pdf-export` so it never appears in the output.
...
- `.tab-nav`, `.pdf-btn`, `.print-btn`, `.save-btn`, `.print-help`, `.add-btn`, `.add-btn-group`, `.delete-btn`, `.scenario-toggle`, `.info-icon`, `#tt-tip` are all `display: none !important`. The `body.pdf-export` block repeats each of these rules so the same elements are hidden in the html2pdf output.
- `.tab-content { display: block !important; }` — every tab is shown in flow. `.tab-content + .tab-content { page-break-before: always; }` puts each tab on its own page. (The `body.pdf-export` block deliberately overrides this to `page-break-before: auto` — see the divergence note above — because html2pdf's slice-based pagination would otherwise emit a mostly-empty page per tab.)
